...
function extract_tags( $html, $tag, $selfclosing = null, $return_the_entire_tag = false, $charset = 'ISO-8859-1' ){
 ...
 function test_links($html)
 {
	 $nodes = extract_tags( $html, 'a' );
	 foreach($nodes as $link){
		if($link['attributes']['href'] != "#link#"){
		     return false;
			}
	}
   return true;
 ...
 function test_imgsrc($html)
 {
 	 $nodes = extract_tags( $html, 'img' );
	 foreach($nodes as $link){
		if($link['attributes']['src'] != "#imagesource#"){
		     return false;
			}
	}
   return true; 
 ...
 function test_heading($html, $tag)
 {	 
 	 $nodes = extract_tags( $html, $tag );
	 foreach($nodes as $link){
	 $taghash = "#".$tag."content#";
		if($link['contents'] != $taghash){			     
		     echo "<br><br>Error here : ".$link['contents'];
		     return false;
			}
	}
   return true; 
 ...
 function test_paragraph($html, $tag)
 {	 
 	 $nodes = extract_tags( $html, $tag );
	 foreach($nodes as $link){
	 $taghash = "#paracontent#";
 ...
 function test_general($html, $tag)
 {	 
 	 $nodes = extract_tags( $html, $tag );
	 foreach($nodes as $link){
	 $taghash = "#".$tag."content#";
 ...
 {	 
	 ...
	 $taghash = "#".$tag."content#";
		if($link['contents'] != $taghash){			     
		     //echo "<br><br>Error here : ".$link['contents'];
			 if(preg_match('/^\<a.*\>.*\<\/a\>/i',$link['contents']))
			 {
			 echo "<br>String contains anchor tag";
